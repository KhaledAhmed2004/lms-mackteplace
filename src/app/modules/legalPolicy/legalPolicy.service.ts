...
export const LegalPolicyService = {
  getAllPolicies: async (): Promise<ILegalPolicy[]> => {
    const result = await LegalPolicy.find().sort({ type: 1 }).lean();
    return result;
  },

  getPolicyByType: async (type: POLICY_TYPE): Promise<ILegalPolicy | null> => {
    const result = await LegalPolicy.findOne({ type }).lean();
    return result;
  },

  getActivePolicyByType: async (type: POLICY_TYPE): Promise<ILegalPolicy | null> => {
  ...
  deletePolicy: async (type: POLICY_TYPE): Promise<ILegalPolicy | null> => {
    const policy = await LegalPolicy.findOne({ type });
  ...
  getAllActivePolicies: async (): Promise<ILegalPolicy[]> => {
    const result = await LegalPolicy.find({ isActive: true }).sort({ type: 1 }).lean();
    return result;
  },

  initializeDefaultPolicies: async (): Promise<void> => {
