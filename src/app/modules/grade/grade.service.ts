...
export const GradeService = {
  createGrade: async (payload: IGrade): Promise<IGrade> => {
  ...
  getAllGrades: async (query: Record<string, unknown>) => {
  ...
  getSingleGrade: async (id: string): Promise<IGrade | null> => {
  ...
  deleteGrade: async (id: string): Promise<IGrade | null> => {
    const grade = await Grade.findById(id);
    if (!grade) {
      throw new ApiError(StatusCodes.NOT_FOUND, 'Grade not found');
    }
