...
export const InterviewSlotService = {
   ...
   */
  markAsCompleted: async (slotId: string): Promise<IInterviewSlot | null> => {
    const slot = await InterviewSlot.findById(slotId);
  ...
  rescheduleInterviewSlot: async (
    ...
    if (!currentSlot) {
      throw new ApiError(
        ...
        'Current interview slot not found',
      );
    }
    ...
    if (newSlot.status !== INTERVIEW_SLOT_STATUS.AVAILABLE) {
      throw new ApiError(StatusCodes.BAD_REQUEST, 'New slot is not available');
    }
   ...
   */
  deleteInterviewSlot: async (id: string): Promise<IInterviewSlot | null> => {
    const slot = await InterviewSlot.findById(id);
   ...
   */
  getMyBookedInterview: async (
    applicantId: string,
  ): Promise<IInterviewSlot | null> => {
    const slot = await InterviewSlot.findOne({
      applicantId: new Types.ObjectId(applicantId),
      status: {
        $in: [INTERVIEW_SLOT_STATUS.BOOKED, INTERVIEW_SLOT_STATUS.COMPLETED],
      },
    })
      .populate('adminId', 'name email')
      .populate('applicationId')
   ...
   */
  getScheduledMeetings: async (query: Record<string, unknown>) => {
    const slotQuery = new QueryBuilder(
    ...
    const meetings = result.map((slot: any) => {
      ...
      const subjectNames =
        application?.subjects?.map((subject: any) =>
   ...
   */
  getInterviewMeetingToken: async (
    slotId: string,
    userId: string,
  ): Promise<{
    token: string;
   ...
   */
  cleanupExpiredAvailableSlots: async (): Promise<number> => {
