...
export const UserService = {
  createUserToDB: async (payload: Partial<IUser>): Promise<IUser> => {
    ...
    const roleLabel =
      createUser.role === USER_ROLES.STUDENT
        ? 'Student'
        : createUser.role === USER_ROLES.TUTOR
          ? 'Tutor'
          : 'User';
  ...
  getUserProfileFromDB: async (user: JwtPayload): Promise<Partial<IUser>> => {
    const { id } = user;
  ...
  updateUserStatus: async (id: string, status: USER_STATUS) => {
  ...
  getAllStudents: async (query: Record<string, unknown>) => {
    const studentQuery = new QueryBuilder(
      User.find({ role: USER_ROLES.STUDENT }).select(
  ...
  unblockStudent: async (id: string) => {
    const user = await User.findById(id);
    if (!user) {
      throw new ApiError(StatusCodes.NOT_FOUND, "User doesn't exist!");
    }
  ...
  adminUpdateStudentProfile: async (
    id: string,
  ...
  getAllTutors: async (query: Record<string, unknown>) => {
  ...
  updateTutorSubjects: async (id: string, subjects: string[]) => {
    const user = await User.findById(id);
    if (!user) {
      throw new ApiError(StatusCodes.NOT_FOUND, "User doesn't exist!");
    }
  ...
  getTutorStatistics: async (
    tutorId: string,
  ): Promise<TutorStatisticsResponse> => {
    // Verify tutor exists
    ...
    const sessionsToNextLevel = getSessionsToNextLevel(
   ...
   */
  updateTutorLevelAfterSession: async (tutorId: string): Promise<void> => {
    const tutor = await User.findById(tutorId);
    if (!tutor || tutor.role !== USER_ROLES.TUTOR) {
      return;
    }
