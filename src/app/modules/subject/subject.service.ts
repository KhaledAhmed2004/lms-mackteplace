...
export const SubjectService = {
  createSubject: async (payload: ISubject): Promise<ISubject> => {
  ...
  getAllSubjects: async (query: Record<string, unknown>) => {
  ...
  getSingleSubject: async (id: string): Promise<ISubject | null> => {
  ...
  deleteSubject: async (id: string): Promise<ISubject | null> => {
    const subject = await Subject.findById(id);
    if (!subject) {
      throw new ApiError(StatusCodes.NOT_FOUND, 'Subject not found');
    }
  ...
  getActiveSubjects: async (): Promise<ISubject[]> => {
